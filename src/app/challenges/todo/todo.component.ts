...
export class TodoComponent implements OnInit {
    ...
    get fc(): Record<string, UntypedFormGroup['controls'][string]> {
    ...
    ngOnInit(): void {
        ...
        if (todos) {
            ...
            this._todos = JSON.parse(todos) as Todo[];
            this.filterTodos();
        }
    }
            ...
            const todo: Todo = {
                id:
                    this._todos.length <= 0
                        ...
                        : this._todos
                              .map(todo => todo.id)
                              .reduce((previous, actual) => (actual > previous ? actual : previous)) + 1,
    ...
    filterTodos(): void {
        if (this.filter === 'all') {
            ...
            this.filteredTodos = this._todos.filter(todo => todo.state === this.filter);
        }
        this.itemsLeft = this._todos.filter(todo => todo.state === 'active').length;
    }
